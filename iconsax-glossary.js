...
(async () => {
  ...
  const state = { query: '', style: 'all', sort: 'az', selected: null, selectedStyle: 'outline', filteredIcons: [], virtualRaf: 0, renderedWindowKey: '', renderedPlaceholderKey: '', visibleIconKeys: new Set() };
  const els = {
  ...
  function card(item, renderIndex = 0) {
    ...
    return `<button class="icon-card card ${hasRevealed ? 'icon-card--ready' : ''}" type="button" data-name="${escapeHtml(item.name)}" title="${escapeHtml(item.name)}" style="--card-delay:${delay}ms">` +
  ...
  function render() { state.filteredIcons = filtered(); state.renderedWindowKey = ''; state.renderedPlaceholderKey = ''; state.visibleIconKeys = new Set(); renderVirtualGrid(); }
